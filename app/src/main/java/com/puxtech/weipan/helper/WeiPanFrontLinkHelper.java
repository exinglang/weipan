 ...
 *
 */
public class    WeiPanFrontLinkHelper  implements BaseLinkHelper{
    /**
     * 目录服务器返回的所有链路
     */
    List<String> allLink;
    ...
    private static WeiPanFrontLinkHelper linkHelper;

    public static WeiPanFrontLinkHelper getInstance(Context context) {
        if (linkHelper == null) {
            linkHelper = new WeiPanFrontLinkHelper(context);
        }
        return linkHelper;
    }

    public WeiPanFrontLinkHelper(Context context) {
//        super(context);
...
//		先增加失败链路
    ...
    public void addFailedLink(String url){
        ...
        Logger.i("已添加微盘前置失败链路：" + url);
    }

    public void clearFailedLink(){
        if(failedLink.size() > 0){
            Logger.i("清除微盘前置已失败链路：count = " + failedLink.size());
